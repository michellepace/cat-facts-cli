...
1. Create your service tokens
   ...
   - Google Developers Platform (For Google Sign in and OAuth)
     ...
     7. In the Authorized redirect URI's section, add two more URLs with the callback URLS like so:
         `https://{what-ever-you-used-as-the-domain-above}.com/auth/google/callback``https://{what-ever-you-used-as-the-domain-above}.com/auth/google/contacts/callback`
...
2. Add tokens and other credentials to your config vars
   ...
   - Add the following config variables to your project (In the Settings tab of the Heroku console and in your development environment if you want to use one):
     ...
     | Name | Value |
     | --- | --- |
     | APIAI\_ACCESS\_TOKEN | {Your api.ai access token} |
     | BASE\_URL | {Your heroku app's domain name} |
     | DB\_USERNAME | {Your mLab database's user account name} |
     | DB\_PASSWORD | {Your mLab database's user account password} |
     | ENCRYPTION\_ALGORITHM | `aes-256-ctr` |
     | ENCRYPTION\_KEY | {Generate a key using the password generator} |
     | GENERAL\_ACCESS\_TOKEN | {Generate a key using the password generator} |
     | GOOGLE\_CLIENT\_ID | {Your Client ID from the Google Developer's Platform |
     | GOOGLE\_CLIENT\_SECRET | {Your Client Secret from the Google Developer's Platform} |
     | IFTTT\_API\_KEY | {The service key provided by the IFTTT Maker Webhooks service} |
     | NODE\_ENV | {"production" for your heroku config and "development" for your own development environment} |
     | SESSION\_SECRET | {Generate a key using the password generator} |
...
1. Set up Tasker automation
   ...
   01. Create a new Profile by clicking the plus button, and choose Time
   02. In the "from" section, choose a time for the Cat Facts to send every day. Then disable the "to" option
   03. Click the back button, and on the next screen click "New Task"
   04. Name the task "CatBot Many" and click the Check box
   05. Add the following actions to this task:
       1. HTTP Get
       ...
       5. Wait
          - Seconds: 2
       6. End For
   06. Go back to the Profiles tab and create an event based profile for "Recieved Text Any"
   07. Create a task for this profile called "CatBot Single"
   08. This task will include these actions:
       1. HTTP Post
          - Server:Port: `{Your heroku app's domain name}`
   ...
   09. Click the back button and exit the app to save changes
...
2. Modify Android sms sending limit
